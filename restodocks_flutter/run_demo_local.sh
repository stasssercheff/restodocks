#!/bin/bash
# Локальный запуск Restodocks Demo (Supabase kzhaezanjttvnqkgpxnh)
# Для проверки «как на сайте» — используй ./build_and_serve_local.sh (собирает как Vercel)

cd "$(dirname "$0")"

# Создай .env.local с:
#   SUPABASE_URL=https://kzhaezanjttvnqkgpxnh.supabase.co
...
if [ -z "$SUPABASE_URL" ] || [ -z "$SUPABASE_ANON_KEY" ]; then
  ...
  echo "  SUPABASE_URL=https://kzhaezanjttvnqkgpxnh.supabase.co"
