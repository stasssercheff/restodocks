...
/** Признак колонки «на 10 порций»: в карточке должны быть брутто/нетто/выход только из расчёта на 1 порцию. */
const PORTION_10_MARKERS = /на\s+10\s+порций|\*10|на\s+10\s+порцию|расход[^]*на\s+10\s+порц/i;

/** Возвращает true, если ячейка (или ячейка выше в предыдущей строке заголовка) относится к блоку «на 10 порций». */
function isColumnFor10Portions(rows: string[][], rowIndex: number, colIndex: number): boolean {
  const cell = (rows[rowIndex]?.[colIndex] ?? "").trim().toLowerCase();
  if (PORTION_10_MARKERS.test(cell)) return true;
  if (rowIndex > 0) {
    const prevRow = rows[rowIndex - 1] ?? [];
    const prevCell = (prevRow[colIndex] ?? "").trim().toLowerCase();
    if (PORTION_10_MARKERS.test(prevCell)) return true;
    if (prevRow.length < (rows[rowIndex]?.length ?? 0)) {
      const prevIdx = Math.min(colIndex, prevRow.length - 1);
      if (prevIdx >= 0 && PORTION_10_MARKERS.test((prevRow[prevIdx] ?? "").trim().toLowerCase())) return true;
    }
  }
  return false;
}
...
export function parseTtkByTemplate(rows: string[][]): TtkCard[] {
  ...
  for (let r = 0; r < rows.length && r < 25; r++) {
    ...
    for (let c = 0; c < row.length; c++) {
      ...
      // Таблицы «на 1 порцию» и «на 10 порций»: брутто/нетто/выход брать только из блока «на 1 порцию», чтобы в итого был выход на 1 порцию.
      if (GROSS_KEYS.some((k) => cell.includes(k))) {
        headerIdx = r;
        if (isColumnFor10Portions(rows, r, c)) continue;
