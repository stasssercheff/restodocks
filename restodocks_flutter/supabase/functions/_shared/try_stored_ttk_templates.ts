...
export async function tryParseByStoredTemplates(rows: string[][], options?: { fromPdf?: boolean }): Promise<TryParseResult | null> {
  ...
  // Системная проблема: "первый попавшийся" header_signature может быть отравлен обучением и ломать парсинг.
  // Поэтому выбираем лучший кандидат: перебираем несколько сигнатур/шаблонов, парсим каждым и берём результат с лучшим score.
  ...
  const templates = (allTemplatesData ?? []) as Array<Record<string, unknown>>;
  if (templates.length === 0) return null;

  const findTemplateForSig = (sig: string): Record<string, unknown> | null => {
    const direct = templates.find((t) => ((t.header_signature as string) ?? "") === sig);
    if (direct) return direct;
    const sigPrefix6 = sig.split("|").slice(0, 6).join("|");
    const match = templates.find((t) => {
      const ts = (t.header_signature as string) ?? "";
      if (!ts) return false;
      if (sig.startsWith(ts + "|") || ts.startsWith(sig + "|")) return true;
  ...
  const headerCandidates: Array<{ sig: string; headerIdx: number; data: Record<string, unknown> }> = [];
  ...
  for (let r = 0; r < rows.length && r < 120; r++) {
    ...
    const data = findTemplateForSig(sig);
    if (data) {
      headerCandidates.push({ sig, headerIdx: r, data: data as Record<string, unknown> });
      seenSig.add(sig);
    }
    if (headerCandidates.length >= 6) break; // достаточно кандидатов, не раздуваем холодный старт
  }
  ...
  let best: { cards: TtkCard[]; sig: string; issues: string[]; score: number } | null = null;

  for (const cand of headerCandidates) {
    const firstSig = cand.sig;
    const data = cand.data;

    // Собираем все строки с тем же заголовком
    ...
    let { cards: allCards, issues: allIssues } = parseBlock(true);
    if (allCards.some(isGarbageCard) && (dishNameRowOffset != null || learnedProductCol != null)) {
      ...
      if (fallback.cards.length > 0 && !fallback.cards.every(isGarbageCard)) {
        ...
        allIssues = fallback.issues;
      }
    }
    if (allCards.length === 0) continue;

    const garbageCount = allCards.filter(isGarbageCard).length;
    const ingrCount = allCards.reduce((s, c) => s + (c.ingredients?.length ?? 0), 0);
    // score: больше карточек/ингредиентов лучше, мусор сильно штрафуем
    const score = (allCards.length * 1000) + ingrCount - (garbageCount * 5000);
    if (!best || score > best.score) {
      best = { cards: allCards, sig: firstSig, issues: allIssues, score };
    }
  }

  if (!best || best.cards.length === 0) return null;
  const uniqueSanity = [...new Set(best.issues ?? [])];
  return { cards: best.cards, headerSignature: best.sig, sanityIssues: uniqueSanity };
}
