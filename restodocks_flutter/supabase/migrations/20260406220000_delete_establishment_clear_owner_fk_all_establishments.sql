-- Админка: удаление заведения через _delete_establishment_cascade.
-- Если у владельца несколько заведений, owner_id на других строках указывает на тот же
-- employees.id — при DELETE employees для одного заведения ломался FK на establishments.owner_id.
...
BEGIN
  ...
  SET owner_id = NULL
  WHERE owner_id IN (SELECT id FROM employees WHERE establishment_id = p_establishment_id);
