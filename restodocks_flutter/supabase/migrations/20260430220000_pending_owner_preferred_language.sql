-- Язык интерфейса при регистрации владельца: сохраняем в pending и переносим в employees при complete.

ALTER TABLE public.pending_owner_registrations
  ADD COLUMN IF NOT EXISTS preferred_language text NOT NULL DEFAULT 'ru';

DROP FUNCTION IF EXISTS public.save_pending_owner_registration(uuid, uuid, text, text, text, text[]);
...
BEGIN
  ...
  VALUES (
    p_auth_user_id, p_establishment_id, trim(p_full_name), nullif(trim(p_surname), ''),
    trim(p_email), p_roles, v_lang, now(), now()
  )
...
REVOKE ALL ON FUNCTION public.save_pending_owner_registration(uuid, uuid, text, text, text, text[], text) FROM PUBLIC;
