-- Обход PostgREST/RLS edge cases: когда PATCH employees даёт 0 строк при «своей» строке
-- (несовпадение id/auth_user_id, политики SELECT/UPDATE, и т.д.).
-- Обновляет ровно одну строку: id = auth.uid() OR auth_user_id = auth.uid().
-- Принимает jsonb с полями как в API; неизвестные ключи игнорируются.
