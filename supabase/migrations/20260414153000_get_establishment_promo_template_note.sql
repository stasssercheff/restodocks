-- get_establishment_promo_for_owner: примечание из promo_codes.note (админка) для подписи в UI.
...
COMMENT ON FUNCTION public.get_establishment_promo_for_owner(uuid) IS
  'Код, срок, флаг отключения, шаблон промокода (тариф, пакеты), примечание из админки; только собственник.';
