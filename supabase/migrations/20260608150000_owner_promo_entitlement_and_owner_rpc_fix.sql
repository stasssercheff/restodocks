-- Fixes:
-- 1) add_establishment_for_owner: Pro entitlement for additional establishments
--    must work when owner has active promo (even if subscription_type is not 'pro').
-- 2) Branch/new establishment should inherit Pro semantics from template with active promo.
-- 3) get_establishment_promo_for_owner should not fail with 400 for owners
--    when there is no owner row in employees for a branch.
...
BEGIN
  ...
  -- Entitlement: paid IAP OR active promo on any owner establishment.
  SELECT EXISTS (
    SELECT 1
...
COMMENT ON FUNCTION public.add_establishment_for_owner(text, text, text, text, text, uuid) IS
  'Add establishment by owner; entitlement includes active promo; copies promo/pro semantics from template.';

GRANT EXECUTE ON FUNCTION public.add_establishment_for_owner(text, text, text, text, text, uuid) TO authenticated;

-- Owner promo info RPC: owner check by establishments.owner_id (not employees row in branch).
...
COMMENT ON FUNCTION public.get_establishment_promo_for_owner(uuid) IS
  'Promo for establishment; owner check by establishments.owner_id.';
