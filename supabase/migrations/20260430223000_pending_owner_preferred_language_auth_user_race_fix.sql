-- Fix: save_pending_owner_registration immediately after signUp.
-- In some cases auth.users row is not visible yet (race), causing auth user mismatch.
...
BEGIN
  ...
  -- Wait briefly for auth.users row to become visible.
  v_auth_exists := false;
  FOR i IN 1..5 LOOP
    SELECT EXISTS (
      SELECT 1
