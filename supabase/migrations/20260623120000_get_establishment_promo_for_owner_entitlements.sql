-- get_establishment_promo_for_owner: добавить шаблон промокода (тариф, пакеты, лимит сотрудников) для UI.
...
COMMENT ON FUNCTION public.get_establishment_promo_for_owner(uuid) IS
  'Код, срок, флаг отключения и шаблон промокода (тариф, пакеты); только собственник.';
