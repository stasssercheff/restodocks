-- Админка вызывает RPC с service_role; при SECURITY INVOKER иногда мешают политики RLS на DELETE.
-- Выполняем каскад от имени владельца функции (postgres), публичный вызов запрещён — только service_role.
...
-- delete_establishment_by_owner (кабинет владельца) тоже вызывает эту функцию.
