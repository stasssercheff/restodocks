...
-- Storage: establishment_id/versions/{id}.xlsx
...
CREATE INDEX IF NOT EXISTS idx_iiko_blank_versions_est_uploaded
  ON iiko_blank_versions(establishment_id, uploaded_at DESC);

ALTER TABLE public.iiko_blank_versions ENABLE ROW LEVEL SECURITY;

-- Anon для Flutter app (custom auth через employees, как inventory_documents)
