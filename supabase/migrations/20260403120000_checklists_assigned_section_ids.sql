-- Несколько цехов у шаблона чеклиста (jsonb-массив кодов KitchenSection).
ALTER TABLE public.checklists
  ADD COLUMN IF NOT EXISTS assigned_section_ids jsonb NOT NULL DEFAULT '[]'::jsonb;

COMMENT ON COLUMN public.checklists.assigned_section_ids IS 'Коды цехов; пустой массив = все цеха. assigned_section дублирует первый элемент для совместимости.';

UPDATE public.checklists
SET assigned_section_ids = CASE
  WHEN assigned_section IS NOT NULL AND btrim(assigned_section) <> ''
  THEN to_jsonb(ARRAY[btrim(assigned_section)])
  ELSE '[]'::jsonb
END
WHERE assigned_section_ids = '[]'::jsonb;

-- Новая сигнатура RPC: p_assigned_section_ids
DROP FUNCTION IF EXISTS public.save_checklist(uuid, text, timestamptz, jsonb, text, text, uuid, jsonb, timestamptz, timestamptz, text, text, jsonb, jsonb);
