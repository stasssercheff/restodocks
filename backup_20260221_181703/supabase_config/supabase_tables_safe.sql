-- Базовые таблицы (безопасно повторно запускать: IF NOT EXISTS).
-- Если при первом запуске было «already exists» — таблицы уже есть, этот скрипт можно просто пропустить.
