...
**Продакшен:** с Hook пользователь получает **два** письма: (1) с PIN/данными из `send-registration-email`, (2) со ссылкой подтверждения из **этого** Hook (русский текст). Без Hook второе письмо идёт шаблоном Supabase (часто на английском). В приложении отдельный Resend на ссылку после `signUp` не дублируется — см. `docs/SUPABASE_AUTH_CONFIRMATION_EMAIL.md`.
